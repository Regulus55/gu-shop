      ...
      <main className={"mx-auto mb-2 max-w-7xl px-4 sm:px-6 lg:px-8"}>
        ...
        <div className="flex items-end justify-between border-b border-gray-200 pt-24 pb-6">
          <div className="flex flex-col">
            {/* page title */}
            <ProductTitle title={"Collections"} />
          </div>
          <div className={"flex items-center"}>
            {/*페이지 드롭다운*/}
            <div
              className={
                "mr-8 text-sm font-medium text-gray-700 hover:text-gray-900"
              }
            >
              Selected Items :{" "}
              <button
            ...
            {[...Array(pageData?.pageCount)].map((_, index) => (
              <button
                ...
                className={`h-10 mx-1 px-4 py-2 bg-violet-300 text-md flex items-center justify-center rounded-lg ${
                  page === index + 1
                    ? "cursor-default scale-125 mx-2 border-2 border-gray-600 pointer-events-none"
                    : "hover:bg-violet-400"
                }`}
              >
                {index + 1}
