...
const CategorySort = ({
  ...
  return (
    <Transition
      ...
      enterTo="transform scale-100 scale-100"
      leave="transform ease-in duration-75"
      leaveFrom="transform opacity-100 scale-100"
      leaveTo="transform opacity-0 scale-95"
    >
      <Menu.Items className="absolute right-0 z-10 w-40 origin-top-right rounded-md bg-white shadow-2xl ring-1 ring-black/5 focus:outline-none">
        <div className="py-1">
          {SORT_OPTIONS.map(({ name, query }) => (
            <Menu.Item key={name}>
              <button
                className={`block w-full px-4 py-2 text-sm hover:bg-gray-100 hover:text-gray-900 ${
                  activeButton === name
                    ? "bg-gray-100 etxt-gray-900"
                    : "text-gray-500"
                }`}
                onClick={() => {
